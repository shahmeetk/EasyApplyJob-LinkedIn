...
def login_LN() -> None:
    ...
    * If both failed, asks user to login manually
    ...
    try:
        ...
        print_lg("Seems like login attempt failed! Possibly due to wrong credentials, human verification required, or already logged in!")
        print_lg("Please login manually in the browser window. The program will wait for you to complete the login process.")
        print_lg("If you see a verification challenge or captcha, please complete it.")
        # print_lg(e)
        # Use a more robust manual login retry that doesn't rely on Tkinter alerts
    ...
    for Question in all_questions:
        ...
        if text:
            ...
            if not prev_answer or overwrite_previous_answers:
                ...
                elif ('hear' in label or 'come across' in label) and 'this' in label and ('job' in label or 'position' in label): answer = "https://github.com/shahmeetk/Auto_job_applier_linkedIn"
...
def external_apply(pagination_element: WebElement, job_id: str, job_link: str, resume: str, date_listed, application_link: str, screenshot_name: str, title: str = "Unknown", company: str = "Unknown") -> tuple[bool, str, int]:
    '''
    ...
    try:
        ...
        apply_button = wait.until(EC.element_to_be_clickable((By.XPATH, ".//button[contains(@class,'jobs-apply-button') and contains(@class, 'artdeco-button--3')]")))
        apply_button.click()
        ...
        print_lg(f"\n*** EXTERNAL APPLICATION LINK CAPTURED ***\nJob: {title} | {company}\nJob ID: {job_id}\nLink: {application_link}\n")
        ...
        error_message = str(e)
        print_lg(f"\n*** FAILED TO GET EXTERNAL APPLICATION LINK ***\nJob: {title} | {company}\nJob ID: {job_id}\nError: {error_message}\n")

        # Take a screenshot for debugging
        if screenshot_name == "Not Available":
            screenshot_name = screenshot(driver, job_id, "Failed to get external application link")

        # Log the failure
        failed_job(job_id, job_link, resume, date_listed, f"Failed to get external application link: {error_message}", e, application_link, screenshot_name)
        global failed_count
        failed_count += 1
            ...
            while current_count < switch_number:
                ...
                for job in job_listings:
                    ...
                    if try_xp(driver, ".//button[contains(@class,'jobs-apply-button') and contains(@class, 'artdeco-button--3') and contains(@aria-label, 'Easy')]"):
                        try:
                            try:
                                ...
                                submit_success = wait_span_click(driver, "Submit application", 2, scrollTop=True)
                                if submit_success:
                                    ...
                                    date_applied = datetime.now()
                                    print_lg(f"\n*** APPLICATION SUBMITTED SUCCESSFULLY (MANUAL CONFIRMATION) ***\nJob: {title} | {company}\nJob ID: {job_id}\n")
                            ...
                            # Take a screenshot for debugging
                            if screenshot_name == "Not Available":
                                screenshot_name = screenshot(driver, job_id, "Failed in Easy Apply process")

                            # Log the failure with detailed information
                            failed_job(job_id, job_link, resume, date_listed, f"Problem in Easy Applying: {error_message}", e, application_link, screenshot_name)
                            failed_count += 1

                            # Try to discard the application if possible
                            try:
                                discard_job()
                            except Exception as discard_error:
                                print_lg(f"Could not discard job application: {discard_error}")
                                try:
                        ...
                        skip, application_link, tabs_count = external_apply(pagination_element, job_id, job_link, resume, date_listed, application_link, screenshot_name, title, company)
                    ...
                    if application_link == "Easy Applied":
                        print_lg(f'\n*** JOB APPLICATION RECORDED SUCCESSFULLY ***\nJob: {title} | {company}\nJob ID: {job_id}\nApplication Type: Easy Apply\n')
                        easy_applied_count += 1
                    else:
                        print_lg(f'\n*** EXTERNAL JOB APPLICATION RECORDED SUCCESSFULLY ***\nJob: {title} | {company}\nJob ID: {job_id}\nApplication Link: {application_link}\n')
    ...
    try:
        ...
        if not os.path.exists(default_resume_path):
            try:
                ...
                print_lg(f"Your default resume '{default_resume_path}' is missing! Will use your previous upload from LinkedIn.")
            useNewResume = False
        ...
        msg = f"\n{quote}\n\n\nBest regards,\nMeet Shah\nhttps://www.linkedin.com/in/meetshah10290/\n\n"
        try:
