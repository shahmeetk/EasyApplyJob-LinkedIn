...
def manual_login_retry(is_logged_in: callable, limit: int = 2) -> None:
    ...
    Note: This function uses pyautogui.alert which requires Tkinter
    '''
    count = 0
    while not is_logged_in():
        try:
            ...
            print_lg(f"Error showing alert: {e}")
            print_lg("Please login manually. The program will wait for you to complete the login process.")
            wait_for_manual_login(is_logged_in)
            return


def wait_for_manual_login(is_logged_in: callable = None) -> None:
    '''
    Function to wait for manual login without using Tkinter alerts
    This is a more robust alternative to manual_login_retry

    Uses the existing browser session to check for login status
    Handles verification challenges and security checks
    '''
    print_lg("Waiting for manual login...")
    print_lg("Please complete the login process in the browser.")
    print_lg("If you see a verification challenge or security check, please complete it.")
    print_lg("The program will continue automatically once you're logged in.")

    # Import the global driver from the main module
    from modules.open_chrome import driver

    # Wait for 5 minutes to allow manual login and verification
    ...
    for i in range(max_attempts):
        ...
        try:
            ...
            if current_url == "https://www.linkedin.com/feed/":
                print_lg("Detected successful login!")
                return

            # Check if we have a is_logged_in function and use it
            if is_logged_in and is_logged_in():
                print_lg("Login successful!")
                return

            # Check for verification challenges
            if "verification" in page_source or "security check" in page_source or "puzzle" in page_source:
                print_lg("\n*** VERIFICATION CHALLENGE DETECTED ***")
                print_lg("Please complete the verification challenge in the browser.")
                print_lg("The program will wait for you to complete it.\n")

            # Check for other common challenges
            if "captcha" in page_source or "robot" in page_source or "human" in page_source:
                print_lg("\n*** CAPTCHA OR HUMAN VERIFICATION DETECTED ***")
                print_lg("Please complete the verification in the browser.")
                print_lg("The program will wait for you to complete it.\n")

        except Exception as e:
            print_lg(f"Error checking login status: {e}")

        print_lg(f"Waiting for manual login... {i+1}/{max_attempts}")
        sleep(5)

    print_lg("Manual login wait time exceeded. Continuing anyway...")
